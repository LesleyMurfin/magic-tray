...
| Stop this mouse on **this PC** so a Mac can use it | Tray -> device row -> **Windows device** -> **Stop this device in Windows** | One consent dialog, then elevated `pnputil /disable-device` on **Bluetooth / HID** nodes for that PID. Stays paired. |
| Start it again on this PC | Same submenu -> **Start this device in Windows** | `pnputil /enable-device` on those same nodes - and nothing at all, with no prompt, when Windows is already using the device. **Only works if Windows still has the device.** |
| Forget / re-pair | Windows **Bluetooth & devices** | Unpair / Add device. **Start this device in Windows** cannot recreate a removed pairing. |
...
| Get a battery percent while the mouse is on the **patched Apple** driver (v3 `0323`) | Tray -> device row -> **Driver:** -> **Read battery now - stops the mouse for 10-20 seconds...** | One elevated cycle, after one OK/Cancel offer that states the cost first: flips the mouse into its battery mode, reads the percent unelevated, then restores the scroll mode and verifies it. One UAC prompt for the whole cycle. Pointer and wheel pause for 10-20 seconds. Never changes which driver is bound. See "Mode A/B battery flip" below. |
| The tray says an earlier battery flip was never confirmed finished, or scroll is dead right after one | Tray -> device row -> **Driver:** -> **Restore scroll mode**, or the startup offer that opens on its own while that is outstanding | One elevated restore of the `LowerFilters` value the tray recorded before it flipped, then a re-read to confirm the scroll mode is back. Does not unpair. |
| Wheel dead while `DEVPKEY_Device_Stack` **already names** the vendor filter | Tray -> device row -> **Scroll problems? Get help** (guidance only), after reading "Which dead-wheel fault is this" below | Not a tray fault and not a registration fault. The mouse never received the Apple multi-touch enable report `{0xF1, 0x02, 0x01}`, which the **driver package's** F1 watcher sends. No tray read can prove that state, so the tray never raises it on its own: you assert the symptom, it names the cause and the two steps. Restarting, reinstalling and re-pairing all do nothing here. |
...
1. Open the tray menu. When something is wrong, the **top row is the problem row** (for example "Scroll wheel is dead - the scroll driver stopped"). When none of those checks finds anything the row reads **No driver or connection problems found** - it is scoped on purpose, because those checks read driver and connection state and never decide whether the **mouse** battery percent is arriving or whether the wheel really scrolls. The one battery state they do report is a connected keyboard reading `-2`, which means Windows is not allowed to read the percent at all (see "What the tray reports per capability - pointer, scroll, battery").
...
| Device is off in the app but present in Windows (`enabled_<pid>=false` while nodes are live) | The tray row was left unchecked. Windows still has the device. | Yes, and it is config-only: it turns **Show in Magic Tray** back on for that row so the device is polled and shown again. No elevation, no `pnputil`, and nothing done to the Windows device - which was never off. |
...
**Why the app used to miss it.** `FilterServiceRunning` comes from `sc.exe query <service>` and is true when that output contains `RUNNING` (`DriverHealthChecker.KernelServiceRunning`). That proves the driver **image** is loaded on this PC. It does not prove the lower filter is **attached** to this mouse's device stack. In this fault the device's `LowerFilters` still names the filter, the filter package is still installed, and the service still reports RUNNING, so every check the app had read healthy - byte-identical to a working mouse.

**The discriminator.** `DEVPKEY_Device_Stack` on the live BTHENUM instance: the ordered list of drivers Windows actually put in the stack. The repo already measured attachment this way in `scripts/capture-state.ps1`, which reads that property on the live BTHENUM instance, joins its string list, and passes the result to `Test-StackHasFilter` to match the filter name against it. The tray now reads the same property from C# (`MagicMouseTray/DeviceStackReader.cs`) and carries the answer on the snapshot as `DeviceSnapshot.FilterInStack`:
...
| Scroll | Registration and attachment as before (rules 1, 2, 2b), plus **counter movement** in the filter's own `Diag` key: `AclTranslateCount`, or `Rid12Count` when that value is absent (`DeviceDiagReader.MultitouchAdvancing`) | That multitouch **is** flowing right now, whenever a counter advanced between two samples | Whether a still counter means a broken mouse or a mouse nobody is touching. Those two are identical reads | Registration faults keep their automatic repairs. A dead wheel with everything attached is **not** auto-detected: the device row carries a user-initiated **Scroll problems? Get help** item that opens the guidance dialog - the same dialog a `RecommendMultitouchWatcher` finding would route to. |
| Battery | The last polled percent for the PID on `DeviceSnapshot.LastBatteryPct`: `-1` no reading - not found, the read timed out, or it answered something that is not a level (a dead or phantom `col02` answering `0`, logged `KB_BATTERY_ZERO`); `-2` present but the battery report is not exposed at all (no Feature capability, or the read itself failed - `KB_BATTERY_BLOCKED` / `KB_FEATURE_BLOCKED`); `-3` three consecutive no-readings | A connected Magic Keyboard whose pairing record is missing the SDP Feature `0x47` capability, which is a real and fixable block | Whether a silent mouse is faulted. An idle mouse legitimately sends nothing at all | Keyboard `-2` -> `BatteryReadBlocked`, routed to the tray's existing **Fix battery reads** SDP patch offer. A keyboard that answered but not with a level is `-1` and raises **nothing** - the report is plainly exposed, so the patch is not the fix. Mouse `-1`, `-2` and `-3` raise **nothing**, deliberately. |

**Pointer: is the Bluetooth pointer child still there.** `DeviceDiagReader.PointerChildLive(pid)` first decides which key under `HKLM\SYSTEM\CurrentControlSet\Enum\HID` even is that PID's pointer child, because two key layouts are real and both were measured on the reference PC (`MagicMouseTray/DeviceDiagReader.ClassifyPointerKey` and `SelectPointerKeys`). A v3 splits its collections **and** keeps a collection-less parent, so `&COL01` wins unconditionally and the parent - an aggregate whose presence reads Unknown, and the wrong devnode to restart - is ignored (`shape=col01`). A v1/v2 exposes exactly one collection, so Windows writes no `&COL0x` suffix at all and that single node is the pointer child (`shape=sole`). No COL01 and two or more distinct collection-less candidates has no principled winner, so nothing is selected (`shape=ambiguous`). Whatever was selected is then resolved with `CM_Locate_DevNodeW(CM_LOCATE_DEVNODE_NORMAL)`: resolved is `true`, a key that exists while no instance of it resolves is `false`, and no candidate at all, an ambiguous field, or any failure is `null`. Only the Bluetooth-transport form of the key counts, `{00001124-...}_VID&...`; `HID\VID_05AC...` and `&MI_` keys are excluded outright. The `DEVICE_DIAG_POINTER` line in `debug.log` carries both the verdict and the shape it came from - on the reference PC, 2026-09-15: `pid=030d live=true keys=1 present=1 shape=sole col01=0 nocol=1`.
...
**Which is why scroll help is user-initiated, not auto-detected.** The scroll line on a device row reads `Scroll: working` when `MultitouchAdvancing` is `true`, and otherwise carries a neutral not-verified wording - `Scroll: driver attached (verified when in use)` where the filter is attached, `Scroll: unknown (cannot confirm it is attached)` where even that is unreadable - never an accusation. The device row also carries **Scroll problems? Get help** (`DeviceCapability.ScrollHelpItemLabel`), offered for a v3 mouse whose bound filter is registered, RUNNING and in the stack, which is the state where every registration rule is already satisfied and fault B is the only remaining explanation. The user asserting the symptom is what removes the false-positive risk that made an automatic version unshippable.

**The item is a question, and that is deliberate.** It reads **Scroll problems? Get help**, not an assertion like "Scroll is not working", because the capability line directly above it may well read `Scroll: working` - a counter genuinely advanced - and an item phrased as a verdict would put the menu in the position of contradicting its own reading two lines earlier (`MagicMouseTray/DeviceCapability.cs:28-33`). For the same reason the item is **not** gated on the observed scroll row: a user reporting that scrolling is wrong anyway is the whole point of the path, so `DeviceCapability.OfferScrollHelp` gates only on the driver-side facts.

**What that dialog says (guidance only).** First step: power the mouse off and on with the underside switch, as three numbered steps (off, wait two seconds, on, then move the pointer and try the wheel). It is the no-risk step, and it is what makes the driver package's watcher see the device arrive and send the enable report. *[Inference: the power-cycle step works because that watcher subscribes to device-arrival events, as described above; the step itself was measured to restore scroll, the arrival mechanism is read from the watcher script, not instrumented.]* Permanent fix: install or restart the driver package's multitouch watcher. The dialog then reports what the tray can already see about that watcher, as **one** tri-state sentence - running on this PC, not running on this PC, or cannot tell - collapsed from `DeviceSnapshot.ScrollWatcherHealthy`, which `DeviceSnapshotReader` derives from `DeviceDiagReader.WatcherState()`: `mm-auto-f1-watcher.ps1` present beside `C:\ProgramData\MagicMouseDriver\auto-f1-watcher.log`, plus that log's newest heartbeat and newest F1 result, with no `schtasks` spawn and no admin. Where the log file exists the dialog also names it as the place to read the watcher's own state by hand. OK opens the driver package's page; nothing is elevated, no device is touched, no script is run.
...
**Battery: one fault worth reporting.** The sentinels the battery readers already return now reach the planner on `DeviceSnapshot.LastBatteryPct` (`-1` no reading - not found, an open failure, a read that timed out, or a read that answered something that is not a level; `-2` present but the battery report is not exposed at all; `-3` the poller collapsing three consecutive `-1`s), and exactly one of them raises a finding: a **connected keyboard** reading `-2`. That rests on `-2` having exactly one meaning: `KeyboardBatteryDevice` returns it only where the Feature capability is missing or `HidD_GetFeature` itself failed, never for a read that answered, so a dead or phantom `col02` answering `0` is `-1` and raises nothing. For the Magic Keyboard that value has one known cause and one known fix - the pairing record's SDP cache does not expose Battery Strength (RID `0x47`) as a Feature report, so `HidD_GetFeature` has nothing to read (`MagicMouseTray/KeyboardBatteryDevice.cs:6-22`). That becomes `BatteryReadBlocked`, and it routes to the offer the tray already had: **Fix battery reads** -> one user-confirmed elevated `scripts/kbd-patch-cachedservices.ps1 -Mac <keyboard MAC>`.
...
|---|---|---|
| Zero instances (v1 `030D` after disable plus **Remove device**), for a PID this PC has an `enabled_<pid>` entry for | The pairing is gone. There is no device node, so there is nothing to enable - `pnputil /enable-device` cannot help. | Flip the underside switch off, then on, until the LED blinks. Windows **Settings -> Bluetooth & devices -> Add device**. Then turn **Show in Magic Tray** back on for the row if it is still off. |
...
| Wheel dead while the bound filter is registered, RUNNING and **in** the device stack - offered only when you click the device row's **Scroll problems? Get help** | Fault B: the mouse never received the Apple multi-touch enable report `{0xF1, 0x02, 0x01}`, so it is still speaking boot-mouse. No tray read can prove this state (see the rejected `LastAclReceived` measurement above), which is why the tray never claims it on its own. | Power the mouse off and on with the underside switch. Then install or restart the driver package's multitouch watcher; the dialog reports whether that watcher looks alive on this PC and, where the file exists, names `C:\ProgramData\MagicMouseDriver\auto-f1-watcher.log`. The tray sends no report, runs no script and elevates nothing. |
...
If the filter is still not running after the BTHENUM restart, the app checks the two conditions that really can stop a self-signed KMDF driver from loading: test signing off (`SystemStartOptions` has no `TESTSIGNING`) or HVCI / Memory integrity on. Only then does the dialog report a block and give the prerequisites as three numbered steps, in order: turn Test Mode on with `bcdedit /set testsigning on` from an administrator Command Prompt, turn Memory integrity off in Windows Security -> Device security -> Core isolation, then reboot and run the fix again. OK opens the Magic Mouse v3 page with the full walkthrough. When test signing is already on and HVCI is already off, the dialog reports a plain failure instead of blaming signing.
...
**The question the tray asks.** Which device gets a Test Mode or Memory integrity fact is decided by the Authenticode signers of the driver file that device actually depends on - never by the PID, and never by how the driver was installed (`MagicMouseTray/SystemConfigChecker.cs`, header comment "WHICH DEVICES GET A SIGNING FACT IS DECIDED BY EVIDENCE, NEVER BY THE PID"). The PID cannot answer it: the same service name and the same file name are used both by Apple's WHQL-signed binary, which loads with nothing switched off, and by a patched, self-signed one. Install provenance cannot answer it either: the sbagirici route puts the genuine Apple-signed file in place by hand, with no INF at all, and needs no Test Mode.

**The reading.** `SystemConfigChecker.ReadBoundDriverSelfSigned` walks every signature in the file's PE certificate table and returns `true` only when every signer is self-issued, `false` as soon as one signature is CA-issued, and `null` when the file carries no embedded signature, is absent, or the read failed. No chain is built, on purpose: a locally installed root would make chain building succeed while the kernel still refused the driver without Test Mode.
...
**What you click.** One action item on the device row's **Driver:** submenu - **Read battery now - stops the mouse for 10-20 seconds...** (`ModeFlipView.MenuItemLabel()`) - and one OK/Cancel dialog before anything happens, which states the 10-20 second pause, the single administrator approval and the guaranteed restore (`ModeFlipView.OfferText`). Cancel there elevates nothing and writes nothing. There is no sticky "battery mode" to be left in: battery mode exists only for the seconds of one reading, and the same submenu carries **Restore scroll mode** as the recovery. The outcome is reported in one dialog per cycle (`TrayApp.ReportModeFlip`), worded per outcome by `ModeFlipView.ResultText`.
...
3. The elevated run flips to Mode A and appends `ready` only once `col02` is actually present. The tray waits up to 45 seconds (`ModeFlip.DefaultTimeoutMs`) for that `ready`, and the restore is always waited for on top of that budget.
...
6. The end state is **verified, not assumed**, and the two halves verify different things. The elevated script writes `restored` only when both its own re-read of `LowerFilters` matches what it recorded **and** the unified no-`&col0x` HID shape is back. The tray's own verdict is narrower and is the one that decides the outcome: `ModeFlip.CompareTargets` re-reads `LowerFilters` for every recorded key and requires the recorded value back verbatim **and in its recorded order** (Windows loads `LowerFilters` in order), case-insensitively per name. That is tri-state: a recorded key that is gone, or a hive that cannot be read, is no evidence and is **not** success. The HID shape is measured and logged beside every verdict (`filters_match=`, `mode_b=`) but deliberately cannot outvote the value - the devnode re-enumerates on the Bluetooth stack's own schedule and was measured arriving after the registry was already correct, so gating on it would report a restore that did land as failed.
7. The sentinel is deleted only when that comparison is true - never on an observed Mode B and never on the script's own `restored` token. The flip also deletes it when the UAC prompt produced no elevated process at all, because then nothing was written and there is nothing to recover from.

Through steps 3 to 7 Windows tears the mouse stack down and rebuilds it, so the pointer and the wheel stop responding for 10-20 seconds - the figure the hardware test measured, and the figure the menu item and the offer dialog both state. That is expected, it is not a fault, and it is over by the time the tray reports the result.

**Failure modes, what you see, and what to do.** Every outcome carries the percent it managed to read, whether the recorded setting was confirmed back (`RestoredToModeB`), and a detail string.
...
| `FlipFailed` | No percent, because battery mode was never confirmed: either `col02` did not appear inside the budget, or the cycle refused to start at all (a recorded filter name that is not a plain service name, a leftover handshake file that would not clear, or a sentinel that could not be written). A cycle that did start restored and re-read as normal; a cycle that refused wrote nothing to restore. | Scroll is fine. Try again later, or read the battery on KMDF instead. |
...
| `RestoreFailed` | The one bad case. The tray does **not** claim success: it reports that it could not confirm the recorded setting came back and that **scroll may be dead**, and it leaves the sentinel in place. | Take **Driver:** -> **Restore scroll mode**: one more UAC prompt, one more verified re-read. If that fails too, Bluetooth & devices -> Remove device, power the mouse off and on, Add device - the measured way to make Windows rebuild the layout. |

So the restore always runs once the elevated script has started, and a restore the tray could not confirm outranks every other result: `Ok`, `FlipFailed` and `BatteryUnreadable` are only reported when the recorded `LowerFilters` value is back, and anything less is reported as `RestoreFailed` however well the reading itself went. `RestoreFailed` is the only outcome that leaves scroll in doubt, and it is never reported as success.

**Crash safety.** The sentinel outlives both the app and the PC. `ModeFlip.StaleModeAOnStartup()` is true whenever that file is still present and parses, which means some earlier cycle never had its restore confirmed: the app was killed, the PC rebooted mid-flip, or a `RestoreFailed` was never cleared (logged `sentinel=kept reason=restore-unverified`). The next start offers the one-click restore on its own - `ModeFlipView.StartupRestoreOffer()`, OK runs `ModeFlip.RestoreModeB()` with its own single UAC prompt and the same verified re-read, Cancel leaves the offer for the next start. The restore writes the recorded names in their recorded order, appends anything the live key carries that the record does not name rather than dropping it, and the sentinel is removed only once the comparison confirms the value. Nothing is retried silently, and nothing is retried in the background.

**What the flip never does.** Its only writes are the `LowerFilters` values it recorded plus the device restart, so it never installs, removes or rebinds a driver package and cannot change which driver you chose. It never unpairs the mouse, never toggles the Bluetooth radio, and never touches a `USB\` or `HID\VID_` phantom node. It never runs on its own: the only entry points are you asking for a reading, you taking **Restore scroll mode**, and you accepting the startup restore offer. And it **never sends F1** - the multitouch enable report remains the driver package's job, exactly as above.

## Tray: Show in Magic Tray, and Windows device

Two different things used to live in one checkbox called "Enabled on this PC", and they are now two items on the device row, because one of them is a PnP change that can fail and the other cannot.

**Show in Magic Tray** is the tray's own switch: whether Magic Tray polls this device and shows its row (`enabled_<pid>` in `config.ini`). Pure user space - no elevation, no PnP, nothing that can fail - so the tick is written immediately, with no confirmation dialog, and the saved value is the only source of the checkbox state. Unticking it stops the polling, not the mouse: the pointer keeps working, the poller skips the PID, and where the row is still drawn its battery reads `Off in this app` rather than claiming a failed read that was never attempted. Log: `DEVICE_SHOW_IN_TRAY pid=... val=true|false`.

**Windows device** -> **Start this device in Windows** / **Stop this device in Windows** is the Windows devnode state, and it is the only part that asks for administrator approval. Two commands rather than a checkbox, because a tick would have to claim a state the app can only sometimes observe. Each one states what it will do before elevating ("It will not move the cursor here until you start it again. It stays paired. This needs administrator approval."), then runs elevated `pnputil /disable-device` or `/enable-device` on the Bluetooth / HID nodes for that PID. The pointer (or keyboard) stops **here**; pairing is unchanged.

Asking to start a device Windows is already driving runs nothing at all: no `pnputil`, no UAC prompt, just "Windows is already using this device, so there is nothing to change." That case is why the split exists. The old single checkbox called `pnputil` first and wrote the tray's own state second, so a Magic Mouse v1 that Windows had never disabled made `pnputil` exit 1, threw before the config write, and left the mouse switched off in the app and unpolled - its row reading `Battery: No reading` for a mouse measured at 97% - while the dialog blamed a UAC prompt the user never saw.

Start it again **only if the device is still in Device Manager / Bluetooth**. If you also **removed** it in Settings, the attempt reports "Windows has no live Bluetooth instance for this device right now, so there was nothing to change. Pair the device and try again.", adds the pairing steps, and offers Bluetooth settings. That is correct: there is nothing to start.

Rows are per **model PID**, not per physical mouse. Two Magic Mouse v1 share one row, one tick and one pair of commands.

USB charge-cable leftovers (`USB\VID_05AC&PID_...`, `HID\VID_...`, `CM_PROB_PHANTOM`) are **not** toggled. Those are not the Bluetooth mouse.

The verdict is the end state, never an exit code: the elevated script reads the device state first, skips `pnputil` entirely when there is nothing to do, and re-reads after any call. Log: `DEVICE_WINDOWS_STATE pid=... enable=... outcome=Changed|AlreadyInState|UacDeclined|Failed|NoInstances` from the tray, and `DEVICE_ENABLE pid=... nonce=... val=...` then `DEVICE_ENABLE pid=... exit=... verified=... sidecar=ok|already|not-verified|no-instances|script-error|none|not-started` from the elevated step. Sidecar is `%TEMP%\mm-enable-<pid>-<procid>-<nonce>.status`, so the result does not depend on UAC `ExitCode`. Both trailing segments are there because two overlapping elevated attempts would otherwise share one status file and the poller could read the other attempt's verdict as its own: `<procid>` is `Environment.ProcessId`, which separates two trays, and `<nonce>` separates attempts inside one tray because `AttemptNonce.Next()` is monotonic and never repeats in a process, where the bare `UtcNow` millisecond it replaced repeated for two attempts that started in the same millisecond. `mm-repair-` and `mm-repair-filters-` carry the same three segments.

## Windows Settings

Tray -> **Bluetooth** -> **Add or change devices…** / **Turn Bluetooth on or off…** opens `ms-settings:bluetooth`; the third item, **Rename a device…**, opens Devices and Printers instead, because renaming a Bluetooth HID device on Windows 11 does not live in Settings. The tray does not pair, does not flip the radio, and does not rename HID devices itself - each item only opens the Windows page that does.
...
4. If tray `config.ini` still has `enabled_030d=false`, tick **Show in Magic Tray** on that row (or delete that line; default is enabled).
...
- **v1 not re-enabling / not discoverable:** PID `030D` absent from PnP and Enum after disable + Settings Remove. `enabled_030d=false`. **Start this device in Windows** cannot help - there is no devnode to start. Pair in pairing mode, then tick **Show in Magic Tray** for the row if it is still off. The tray problem row now says exactly that instead of offering a start that cannot work.

...
# WEAK admissions: ordinary English that only concedes a defect when something
# else in the sentence defers the work. "left alone" is the whole reason this
# distinction exists — it false-positived on "a non-interactive seat is now left
# alone", a sentence describing DESIGNED behaviour, which is the opposite of an
# admission. The phrase is kept in the vocabulary (real admissions do use it)
# but it must now be corroborated.
#
# Every other alternative above concedes on its own: "out of scope" and "not
# fixed" cannot describe intended behaviour, so they are unchanged and still
# block by themselves.
_WEAK_ADMISSION_RE = re.compile(
    r"^(?:left\s+alone|left\s+as[-\s]is)$",
    re.IGNORECASE,
)

# What turns a weak phrase into an admission: a marker that the work is being
# PUT OFF rather than described. Searched within the same sentence as the match.
_DEFERRAL_RE = re.compile(
    r"""\b(
        for\s+now
      | for\s+the\s+moment
      | for\s+the\s+time\s+being
      | in\s+this\s+(?:pr|change|patch)
      | this\s+time
      | later
      | pending
      | until\b
      | revisit
      | to\s+be\s+(?:fixed|done|handled|addressed)
      | should\s+be\s+(?:fixed|done|handled|addressed)
      | needs?\s+(?:fixing|work|attention)
      | deliberately\s+unfixed
    )""",
    re.IGNORECASE | re.VERBOSE,
)
...
def _sentence_around(line: str, start: int, end: int) -> str:
    """The sentence containing ``line[start:end]``.

    Corroboration is sentence-scoped on purpose: a deferral marker belonging to
    a different clause on the same line says nothing about this phrase.
    """
    left = line.rfind(".", 0, start)
    right = line.find(".", end)
    return line[left + 1 : right if right != -1 else len(line)]


def _strip_inline_code(text: str) -> str:
    """Blank out `inline code` spans.

    A deferral marker inside backticks is being QUOTED, not asserted. Without
    this, a body that documents the vocabulary — "they convict only when the
    sentence also defers the work (`for now`, `in this PR`)" — corroborates
    itself and is convicted for explaining the rule. Applied only to the
    corroboration search, so it can never suppress a strong admission.
    """
        ...
        for offset, line in enumerate(section.split("\n")):
            ...
            for match in _ADMISSION_RE.finditer(line):
                ...
                if _DEICTIC_RE.match(line[match.end() :]):
                    continue
                # A weak phrase is ordinary English unless the same sentence
                # also defers the work. Without this, "is now left alone"
                # (designed behaviour) reads identically to "left alone for
                # now" (an unfiled defect).
                if _WEAK_ADMISSION_RE.match(match.group(0)) and not _DEFERRAL_RE.search(
                    _strip_inline_code(
                        _sentence_around(line, match.start(), match.end())
                    )
                ):
                    continue

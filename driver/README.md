# KMDF does not live here

`LesleyMurfin/magic-tray` is the tray client. It does **not** vendor `MagicMouseDriver` source, INF, vcxproj, `.sys`, or sign/install scripts.

**KMDF home:** https://github.com/LesleyMurfin/magic-mouse-v3-windows-fix (`v2-kmdf-driver/`)

The tray **pulls that repo** when the user selects Best / MagicMouseDriver for PID **0323**, then runs **that repo’s KMDF one-click** (sign/install / SYSTEM task under `v2-kmdf-driver/`). It does not copy those files into this folder, does not generate `bind-filter.ps1`, and does **not** run PATH-A `v1-binary-patch/installer/Install-MagicMousePatch.ps1`.

| Device | What the tray does |
|--------|-------------------|
| 0323 | Pull `magic-mouse-v3-windows-fix` and run **their KMDF** installer in `v2-kmdf-driver/` |
| 030D / 0269 / 0310 | Pull tealtadpole Boot Camp INF (do not retarget with KMDF) |
| Keyboard | PATH-C SDP patch (`scripts/kbd-patch-cachedservices.ps1`) — not Keymagic2 |

If `v2-kmdf-driver/` has no one-click script, install reports that and stops. Do not fall back to PATH-A. Do not vendor source or scripts here.
